function HideDiv()
{
	for(var i=1; i<12; i++)	document.getElementById('sel'+i).style.visibility = 'hidden';
}

function ShowDiv()
{
	for(var i=1; i<12; i++)	document.getElementById('sel'+i).style.visibility = 'visible';
}
...
function pickColor(color){if(ColorPicker_targetInput==null){alert("Target Input is null, which means you either didn't use the 'select' function or you have no defined your own 'pickColor' function to handle the picked color!");return;} ColorPicker_targetInput.value = color; ShowDiv(); ChangeTpl();}
function ColorPicker_select(inputobj,linkname){HideDiv(); if(inputobj.type!="text" && inputobj.type!="hidden" && inputobj.type!="textarea"){alert("colorpicker.select: Input object passed is not a valid form input object");window.ColorPicker_targetInput=null;return;}window.ColorPicker_targetInput = inputobj;this.show(linkname);}
...
function ColorPicker()
{
	...
	{
		var cp = new PopupWindow();cp.setSize(225,250);
	}
